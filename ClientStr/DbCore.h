...
namespace NoSqlDb
{
  ...
  public:
	...
	void nameSpace(const std::string& name) { nameSpace_ = name; }
	...
	//check-in version number vector
	std::vector<int>& version() { return version_; }
	std::vector<int> version() const { return version_; }
	void version(const std::vector<int>& version) { version_ = version; }
